...
更关键的是我们有**反向实证**：曾自建过更复杂的"框架化"架构（双核：Flash 主循环 + Pro 规划/反思 + 元认知路由 + 契约 + 认知黑板），结果路由不可靠、主循环死循环、补丁叠补丁，最终整体回退（`git reset --hard d4b00c8`，备份 `backup-pre-dualcore-20260726`）。这证明哪怕是自己写的框架抽象叠在简单 loop 上也会爆炸——外部框架只是把这种不透明和补丁成本外包出去，还附赠依赖与版本漂移。候选框架对比与完整理由见 `docs/框架选型决策.md`。
